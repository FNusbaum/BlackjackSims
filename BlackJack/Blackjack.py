...
class Game():
    ...
    def test_strats(self):
        ...
        for day in range(10000):
            Days += [day]
            arr =  self.run_games(120)
            if sum(self.profit) > 0:
                plus_count+=1
            else:
                minus_count+=1
            Daily_profits += [sum(self.profit)]
            prof_per_bets = sum(self.profit) / sum(self.bet_tracker)
            Daily_profs_per_bets += [prof_per_bets]
            Daily_bets += [sum(self.bet_tracker)]

            self.reset_day()
        #print(f"Risk Of Ruin: {100*ruin_count / 10000}%")
        #print(f"Prob Of 2.5K: {100*tenK_count / 10000}%")
        print(f"Prob of Win: {plus_count/10000}")
        print(f"Prob of Los: {minus_count/10000}")
        average_prof_per_bets = round(sum(Daily_profs_per_bets)/len(Daily_profs_per_bets),4)
        average_profits = round(sum(Daily_profits)/len(Daily_profits),4)
        sd_profs = 0
        for num in Daily_profits:
            sd_profs += ((num - average_profits)**2)/len(Daily_profits)
        sd_profs = round(sd_profs**(1/2),4)
        min_profs = min(Daily_profits)
        max_profs = max(Daily_profits)
        ave_daily_bets = round(sum(Daily_bets)/len(Daily_bets),2)
        #plt.scatter(Days,Daily_profits, s=5)
        #plt.show()

        return [average_prof_per_bets, average_profits,ave_daily_bets,sd_profs,min_profs,max_profs]
...
if "__main__" == __name__:
    ...
    for i in range(n):
        X+=[1]
        Start_of_hand_count = Game.true_count
        if Start_of_hand_count not in Game.count_results.keys():
            Game.count_results.update({Start_of_hand_count: [0,0,0,0,0]})
        Game.count_results[Start_of_hand_count][0] += 1
        Game.play_hand()
        new_profit = sum(Game.profit)
        if new_profit == overall_profit:
            Game.count_results[Start_of_hand_count][0] += -1
        elif new_profit > overall_profit:
            Game.count_results[Start_of_hand_count][1] += 1
            Game.count_results[Start_of_hand_count][4]= round(100*Game.count_results[Start_of_hand_count][2]/Game.count_results[Start_of_hand_count][0],2)
            Game.count_results[Start_of_hand_count][3] = round(100*Game.count_results[Start_of_hand_count][1]/Game.count_results[Start_of_hand_count][0],2)
        else:
            Game.count_results[Start_of_hand_count][2] += 1
            Game.count_results[Start_of_hand_count][4]= round(100*Game.count_results[Start_of_hand_count][2]/Game.count_results[Start_of_hand_count][0],2)
            Game.count_results[Start_of_hand_count][3] = round(100*Game.count_results[Start_of_hand_count][1]/Game.count_results[Start_of_hand_count][0],2)
        overall_profit = new_profit
        winnings += [overall_profit]
        if i %200 == 0:
            Game.reset_day()
    keys = Game.count_results.keys()
    print("count, total, Ws, Ls, W%, L%")
    for key in sorted(keys):
        print(key, Game.count_results[key])
    print("%HandsWithDeviations:", Game.num_play_deviations/n)
    print(f"winCount, %: {Game.numWins}, {Game.numWins/n}" )
    print(f"lossCount, %: {Game.numLosses}, {Game.numLosses/n}" )
    print(f"pushes, %: {n-Game.numLosses-Game.numWins}, {(n-Game.numLosses-Game.numWins)/n}" )
    print(f"CasinoEdge: {((Game.numLosses/n)-(Game.numWins/n))*100}%")
...
'''
    start  = time.time()
    Game = Game(6, 1)
    #for each strategy, returns: [profit/bankroll, average_profit, sd_profit, min_profit, max_profit]
    bet_strategy_results = {} 
    #print("average_prof_per_bankroll, average_profits,sd_profs,min_profs,max_profs")
    arr = Game.test_strats()
    print(f"prof/bets: {arr[0]}")
    print(f"aveProfit: {arr[1]}")
    print(f"aver_Bets: {arr[2]}")
    print(f"Stand_dev: {arr[3]}")
    print(f"MinProfit: {arr[4]}")
    print(f"MaxProfit: {arr[5]}")
    print()
